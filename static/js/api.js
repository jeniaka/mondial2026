...
export const api = {
  ...
  groupRename:(id, name)         => fetchJson(`/api/groups/${id}`, { method: 'PATCH', body: { name } }),
  groupRegenCode:(id)            => fetchJson(`/api/groups/${id}/regenerate-code`, { method: 'POST' }),
  groupReset: (id)               => fetchJson(`/api/groups/${id}/reset`, { method: 'POST' }),
  groupDelete:(id)               => fetchJson(`/api/groups/${id}`, { method: 'DELETE' }),
  groupTransfer:(id, userId)     => fetchJson(`/api/groups/${id}/transfer`, { method: 'POST', body: { user_id: userId } }),
  groupMute:  (id, muted)        => fetchJson(`/api/groups/${id}/mute`, { method: 'POST', body: { muted } }),
  groupStats: (id)               => fetchJson(`/api/groups/${id}/stats`),
